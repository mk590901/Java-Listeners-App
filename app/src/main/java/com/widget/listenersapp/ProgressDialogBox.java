...
public class ProgressDialogBox {
    ...
    public ProgressDialogBox(final Activity activity,
                             final String title,
...
// Set up the dialog
        Context context = new ContextThemeWrapper(mActivity, R.style.Theme_ListenersApp);
...
//  Show the dialog
    ...
    public void start() {
        ...
        mHandler.postDelayed(() -> {
            boolean connectionOk = true;//getConnectionResult();    // false;
            if (connectionOk) {
                close(true);
            }
            else {
